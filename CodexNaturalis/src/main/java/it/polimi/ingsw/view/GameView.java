...
public interface GameView extends ReconnectableView{
    ...
    /**
     * A method that prints (pops out if the view is of graphical type) a message indicating
     * that the turn players are going to play next, will also be the last.
     */
    void notifyLastTurn();
    ...
    /**
     * Prints the lastly received chat message.
     * This print, can vary dependently on who sent the message to whom.
     *
    ...
    /**
     * Changes the current turn owner and notifies (prints in CLI view, pops out in GUI view) the player of that.
     *
    ...
    /**
     * Shows (prints in CLI view, pops out in GUI view) the user that an error has occurred
     *
    ...
    /**
     * Notifies (prints in CLI view, pops out in GUI view) the player that a player has joined the game.
     ...
     * From the point of view of the local player, the hand cards of the enemies are composed only by backs.
     *
     ...
     * @param placedCards   the list of placed cards (aka the board) of the specified player.
     ...
     * These objectives are secret, and the enemies cannot se them.
     *
    ...
    /**
     * Updates the current decks viewing.
     * These are intended (here) as the currently drawable cards, not the entire decks.
     *
     * @param drawableCards     the map linking a CardType to its drawable BasicCards.
     * @param numberOfCardsLeft the number of card left in each deck.
     *
    ...
    /**
     * Reveals the final game summary of one player, which includes their score obtained objective by objective and,
     * ultimately, their final total score.
     *
     ...
     * This occurs when server detected that a client is not acknowledging pings.
     *
    ...
    /**
     * Notifies that the game has aborted, by showing a message.
     * Then returns to the setup phase.
     */
